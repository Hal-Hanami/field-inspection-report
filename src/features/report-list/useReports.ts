 ...
 */
export function useReports(): { reports: InspectionReport[]; loading: boolean; failed: boolean } {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // §4.7 — an unreachable server is said, not shown as loading that never ends.
  ...
  return { reports: reports ?? [], loading: reports === null && !failed, failed };
}
